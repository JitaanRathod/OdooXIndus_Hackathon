const { Transfer, TransferLine, Product, Location, User, Inventory, StockMove, sequelize } = require('../models');
...
const confirm = async (id, userId) => {
  ...
  await sequelize.transaction(async (t) => {
    for (const line of transfer.lines) {
      ...
      await fromInv.update({ qty_on_hand: parseFloat(fromInv.qty_on_hand) - qty }, { transaction: t });
      ...
      await toInv.update({ qty_on_hand: parseFloat(toInv.qty_on_hand) + qty }, { transaction: t });
      ...
      await StockMove.create({
        ...
        user_id: userId,
        notes: `Transfer ${transfer.reference_no} confirmed`,
      }, { transaction: t });
    }
    await transfer.update({ status: 'confirmed', confirmed_by: userId }, { transaction: t });
  });
