const { Delivery, DeliveryLine, Product, Location, User, Inventory, StockMove } = require('../models');
const AppError = require('../utils/AppError');
const { nextRef } = require('../utils/refGen');

const list = () => Delivery.findAll({
...
const getOne = async (id) => {
  const d = await Delivery.findByPk(id, {
    include: [
      { model: DeliveryLine, as: 'lines', include: [{ model: Product, as: 'product' }, { model: Location, as: 'location' }] },
...
const create = async ({ customer_name, notes, lines }, userId) => {
  ...
  if (lines?.length) {
    await DeliveryLine.bulkCreate(lines.map(l => ({
      delivery_id: delivery.id, product_id: l.product_id,
      location_id: l.location_id, qty: l.qty,
    })));
  }
  return delivery;
};
...
const validate = async (id, userId) => {
  ...
  for (const line of delivery.lines) {
    ...
    const inv = await Inventory.findOne({ where: { product_id: line.product_id, location_id: line.location_id } });
    if (!inv || parseFloat(inv.qty_on_hand) < qty) {
      throw new AppError(`Insufficient stock for product: ${line.product?.name || line.product_id}`, 400);
    }
    await inv.update({ qty_on_hand: parseFloat(inv.qty_on_hand) - qty });
    await StockMove.create({
      product_id: line.product_id, from_location_id: line.location_id,
      to_location_id: null, qty,
      type: 'delivery', reference_id: delivery.id, reference_type: 'delivery',
      user_id: userId, notes: `Delivery ${delivery.reference_no} validated`,
    });
  }
  return delivery.update({ status: 'done', validated_by: userId, validated_at: new Date() });
};
