const { Receipt, ReceiptLine, Product, Location, User, Inventory, StockMove, sequelize } = require('../models');
...
const validate = async (id, userId) => {
  ...
  // Wrap all inventory mutations in a single atomic transaction
  await sequelize.transaction(async (t) => {
    for (const line of receipt.lines) {
      // Use qty_received if provided (partial delivery), else fall back to qty_expected
      const qty = parseFloat(line.qty_received ?? line.qty_expected);
      ...
      await line.update({ qty_received: qty }, { transaction: t });
      await StockMove.create({
        ...
        user_id: userId,
        notes: `Receipt ${receipt.reference_no} validated`,
      }, { transaction: t });
    }
...
const cancel = async (id) => {
  ...
  if (['done', 'cancelled'].includes(receipt.status))
    throw new AppError('Cannot cancel this receipt', 400);
